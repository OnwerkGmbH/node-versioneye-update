...
function loadOptions() {
    ...
    // *** Load cliOptions from the config file given in command line
    if (parsedCommandLineArguments.configfile) {
        ...
        if (parsedCommandLineArguments.file) {
            ...
            if (directoryExists(optionFileDir)) {
                tryCwd = false;

                var cliOptionFilePath = path.join(optionFileDir, constants.configFileName);

                if (fileExists(cliOptionFilePath)) {
                    cliOptions = tryLoadOptionsFile(cliOptionFilePath, parsedCommandLineArguments.dump, cliOptions, '(projectfile)');
                }
            }
        }
        ...
        if (tryCwd) {
            var cliOptionFilePathCwd = path.join(process.cwd(), constants.configFileName);
            cliOptions = tryLoadOptionsFile(cliOptionFilePathCwd, parsedCommandLineArguments.dump, cliOptions, '(cwd)');
        }
    }
...
function createOptionFile(parsedBody) {
    ...
    if (cliOptions.globalinstalls === true) {
    ...
    if (cliOptions.globalinstalls === true) {
        console.log(colors.green(messages.storeThisFileLocally));
    } else {
        console.log(colors.green(messages.addThisFileToRepository));
    }
}
...
function uploadGloballyInstalledPackages() {
    ...
    if (npmOutput === false || npmOutput.dependencies === false) {
