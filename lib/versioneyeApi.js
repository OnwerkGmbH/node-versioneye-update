 ...
 * @return {undefined}
 */
VersioneyeApi.prototype.updateProject = function(baseUrl, projectId, filePath, callback) {
    var formData = {
        project_file: fs.createReadStream(filePath)
    };

    request.post({
        url: util.format(config.uri, baseUrl, projectId, this.apiKey),
        formData: formData
